...
module.exports = React.createClass({
	...
	render() {
		...
		var header = this.props.expanded ? (<div className={componentClass}>
				<span className="date-select-calendar-header-dow">{date.format('dddd')}</span>
				<span className="date-select-calendar-header-month">{date.format('MMMM')}</span>
				<span className="date-select-calendar-header-day">{date.format('D')}</span>
				<span className="date-select-calendar-header-year">{date.format('YYYY')}</span>
			</div>) : ( <div className={componentClass}>
				<span className="date-select-calendar-header-dow">{date.format('dddd')}</span>
				<span className="date-select-calendar-header-day">{date.format('Do')}</span>
				<span className="date-select-calendar-header-month">{date.format('MMMM')}</span>
				<span className="date-select-calendar-header-year">{date.format('YYYY')}</span>
			</div>);
